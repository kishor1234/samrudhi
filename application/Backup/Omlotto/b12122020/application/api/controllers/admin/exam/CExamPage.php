...
class CExamPage extends CAaskController {
    ...
    public function initialize() {
        parent::initialize();
        $data=array();
        $sql = $this->ask_mysqli->select("exam", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("id" => $_POST["id"])); // . $this->ask_mysqli->limitWithOffset($this->start, $this->end);
        $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
        while ($row = $result->fetch_assoc()) {
            $dt = $this->getLogo($row["companyid"]);
