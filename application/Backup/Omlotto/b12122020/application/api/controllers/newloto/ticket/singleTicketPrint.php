...
class singleTicketPrint extends CAaskController {
    ...
    public function printTicket() {
        ...
        if ($row = $result->fetch_assoc()) {
                    ...
                    foreach ($_1array as $_2key => $_2array) {//1800
                        ...
                        foreach ($_2array as $in => $valArray) {//1800
                            foreach ($valArray as $index => $value) {//point
                                $indV = $_2key + $index;
                                $v = $v + $value;
                                array_push($temp, array("srno" => $i, "drDate" => $row["enterydate"], "drTime" => $row["gametimeid"]."[".date( 'g:i A', strtotime( $row["gameendtime"] ) )."]", "name" => "RajLaxmi", "mrp" => ($value * 2), "digit" => $indV, "qty" => $value));
                                $i++;
                            }
                        }
                    }
            ...
            echo json_encode(array("status" => "1", "msg" => "This is to certify that,the Cash Memo/Invoice (T. No.{$row["utrno"]} OR Barcode No. {$row["game"]}- of Rs. {$request["amount"]}) colud not be printed,\n I hereby to be required reprinting the same. I indentify for any misuse of duplicate Cash Memo/Invoice.", "point" => $temp));
        } else {
